...
async function renderHome() {
  ...
  app.innerHTML = `
    <div class="min-h-screen">
      ...
      <section class="max-w-6xl mx-auto px-4 py-12">
        <h2 class="text-2xl font-bold text-slate-800 mb-6">開催中のイベント</h2>
  ...
  // イベント取得
  try {
    const data = await window.LinkUpAPI.Events.list({ limit: 50 });
...
function _renderEventCard(ev) {
  ...
  return `
    <div class="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden cursor-pointer group"
         onclick="AppRouter.go('detail', { id: '${ev.event_id}' })">
      <div class="relative h-48 overflow-hidden">
        ...
        ${ev.category ? `<span class="absolute top-3 left-3 px-2 py-1 rounded-full text-xs font-bold ${categoryColor}">${ev.category}</span>` : ''}
      </div>
      <div class="p-5">
        <h3 class="font-bold text-slate-800 text-base mb-2 line-clamp-2">${ev.title}</h3>
        <div class="space-y-1.5 text-sm text-slate-500">
          ...
          <p class="flex items-center gap-1.5">
            ...
            <span class="truncate">${ev.venue_name || 'オンライン'}</span>
          </p>
        ...
        <div class="mt-4 flex items-center justify-between">
          <span class="text-xs text-slate-400">${ev.organizer_name || ''}</span>
